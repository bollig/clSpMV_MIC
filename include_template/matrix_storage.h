...
public:
	...
	~bcsr_matrix() {
		...
		//if (bcsr_data)    {delete [] bcsr_data_ptr; bcsr_data_ptr = 0;} // GE ERROR
		if (bcsr_data)    {delete [] bcsr_data; bcsr_data = 0;}
	}
};
...
public:
	...
	~bell_matrix() {
    	if (bell_col_id) free(bell_col_id);
...
// On Nvidia Divice, the global memory should be padded to a multiple of 128 bytes, or 32 floats. 
template <class dimType, class dataType>
bool coo2b4csr( coo_matrix<dimType, dataType>* source, 
				b4csr_matrix<dimType, dataType>* dest, 
				unsigned int bwidth, unsigned int bheight, dimType alignment)
{
    using namespace std;
    assert(bwidth % 4 == 0);
    if (!if_sorted_coo<dimType, dataType>(source))
...
void coo2bell(coo_matrix<dimType, dataType>* source, bell_matrix<dimType, dataType>* dest, unsigned int bwidth, unsigned int bheight, dimType alignment, dimType ellnum)
{
    ...
    dest->bell_col_id = new dimType [newlength*ellnum]; 
    dest->bell_data = new dataType [newlength*ellnum*bwidth*bheight];
...
// On Nvidia Divice, the global memory should be padded to a multiple of 128 bytes, or 32 floats. 
template <class dimType, class dataType>
bool coo2b4ell( coo_matrix<dimType, dataType>* source, 
				b4ell_matrix<dimType, dataType>* dest, 
				unsigned int bwidth, unsigned int bheight, dimType alignment, dimType ellnum)
{
    using namespace std;
    assert(bwidth % 4 == 0);
    if (!if_sorted_coo<dimType, dataType>(source))
