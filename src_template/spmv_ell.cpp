...
void spmv_ell_ocl_T(ell_matrix<int, T>* mat, T* vec, T* result, int dim2Size, double& opttime, int& optmethod, char* oclfilename, cl_device_type deviceType, T* coores, int ntimes)
{
	...
	if (sizeof(T) == sizeof(float) {
		...
		csrKernel = clCreateKernel(program, "gpu_ell_v4_d", &errorCode); CHECKERROR;
	}
	//csrKernel = clCreateKernel(program, "gpu_ell_v4", &errorCode); CHECKERROR;
	errorCode = clSetKernelArg(csrKernel, 0, sizeof(cl_mem), &devColid); CHECKERROR;
	errorCode = clSetKernelArg(csrKernel, 1, sizeof(cl_mem), &devData); CHECKERROR;
...
#if 0
template <typename  T>
void spmv_ell_T(char* oclfilename, coo_matrix<int, T>* mat, int dim2Size, int ntimes, cl_device_type deviceType)
{
    printMatInfo(mat);
    ell_matrix<int, T> ellmat;
    coo2ell<int, T>(mat, &ellmat, GPU_ALIGNMENT, 0);
    T* vec = (T*)malloc(sizeof(T)*mat->matinfo.width);
    T* res = (T*)malloc(sizeof(T)*mat->matinfo.height);
    initVectorOne<int, T>(vec, mat->matinfo.width);	
    initVectorZero<int, T>(res, mat->matinfo.height);
    T* coores = (T*)malloc(sizeof(T)*mat->matinfo.height);
    spmv_only_T<T>(mat, vec, coores);

    {
	double opttime1 = 10000.0f;
	int optmethod1 = 0;

	spmv_ell_ocl_T(&ellmat, vec, res, dim2Size, opttime1, optmethod1, oclfilename, deviceType, coores, ntimes);
