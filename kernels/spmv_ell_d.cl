...
__kernel void gpu_ell_d(__global int* col_id, __global double* data, int aligned_length, int ell_num, __global double* vec, __global double* result, int row_num)
{
	...
	double matrixelem = data[matoffset];
...
__kernel void gpu_ell_v4_d(__global int4* col_id, __global double4* data, int aligned_length, int ell_num, __global double* vec, __global double4* result, int row_num)
{
    int row = get_global_id(0);
    //double4 accumulant = result[row];
    double4 accumulant = {0.,0.,0.,0.};
    int matoffset = row;
    for (int i = 0; i < ell_num; i++)
    {
