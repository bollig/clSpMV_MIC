  ...
  namespace ocl
  {
    ...
    void enqueue(KernelType & k, viennacl::ocl::command_queue const & queue)
    {
      ...
      else //2D kernel
      {
        ...
        cl_int err = clEnqueueNDRangeKernel(queue.handle().get(), k.handle().get(), 2, NULL, tmp_global, tmp_local, 0, NULL, &event);
...
// START ADDED BY GORDON ERLEBACHER
// CONSIDER a flag: if benchmark
	int err = clWaitForEvents(1, &event);
	if (err != CL_SUCCESS) {
        printf("[enqueueKernel] ERROR in clWaitForEvents\n");
		exit(1);
	}
	...
	size_t  start_ret;
	size_t end_ret;
	err = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_START, sizeof(unsigned long), &start, &start_ret);
	if (err != CL_SUCCESS) {
        printf("[enqueueKernel] ERROR in clGetEventProfilingInfo\n");
		exit(1);
	}
	err = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_END,   sizeof(unsigned long), &end, &end_ret);
	if (err != CL_SUCCESS) {
        printf("[enqueueKernel] ERROR in clGetEventProfilingInfo\n");
		exit(1);
	}
...
// END ADDED BY GORDON ERLEBACHER
